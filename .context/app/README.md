...
| `prisma/schema/app.prisma` + `app_…` migrations | **Shared** app-domain file — Sunrise ships its own app-tier models here too (`ContactSubmission`, `FeatureFlag`, `AuthBootstrap`), so add HCE Hub models _alongside_ them, `app_*`-prefixed to namespace; migrations touch only `app_*` tables. (Sunrise [#429](https://github.com/human-centric-engineering/sunrise/issues/429) tracks clarifying this upstream.) |
