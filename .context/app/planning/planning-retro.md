...
### HB10 · A guard that introduces a new failure mode must ship that mode's remedy in the same task — an error telling the caller to "retry" with nothing retrying is advice, not a mechanism (HCE Hub · f-authoring-fidelity)

- **Discovery.** `t-87` closed a TOCTOU in the dependency-cycle guard: the graph read moved inside the write transaction, and the transaction was raised to `Serializable` (moving the read in is necessary but not sufficient — under Read Committed both writers still validate clean and both still commit). That fix **introduced a failure mode the verb never had**: the Postgres SSI abort, surfaced by Prisma as `P2034`. I mapped it to a caller-facing `concurrent_modification` whose message read _"re-read the task and retry"_ — and implemented no retry. It took **two** review passes to land: the first caught that `Serializable` had been applied to every update rather than the edge path, so a plain title edit now failed where it used to wait on a row lock; the second caught that even on the correct path the abort was surfaced but never absorbed.
- **Impact.** The guard traded a **rare correctness bug** (two concurrent writers closing a cycle) for a **routine liveness bug** (an edit silently dropped), which is a bad trade in both directions — and the second is harder to notice, because nothing errors *loudly*: the caller gets a plausible message describing a remedy it cannot perform. The scope slip made it worse by extending the new failure mode to writes that had no race to lose. The most uncomfortable part is that the error message **was** the tell: I wrote the correct remedy as prose in the same commit where I declined to build it, which means the design was understood at authoring time and simply not finished. A reviewer had to supply what the message already knew.
- **Feedback.** When a task adds a defensive mechanism — an isolation level, a lock, a constraint, a uniqueness guard, a validation gate — enumerate the failure modes it **newly introduces**, and for each name **who handles it, in the same task**. A mechanism's error path is part of the mechanism, not a follow-up; shipping the throw without the catch moves the cost onto whoever hits it first. Two concrete tells: (1) you are writing remediation advice into an error message ("retry", "re-read", "try again later") — ask whether the caller can act on it, and remember that when the caller is an **LLM agent over MCP**, prose advice is not a mechanism and there is no framework retry behind it; (2) you are applying the guard **uniformly** rather than to the path that needs it — scope it, because a guard applied where there is nothing to guard is purely a regression. Cross-ref [HB4] (confirm a guard's trigger is reachable) — this is its mirror: confirm a guard's *consequences* are handled. _Status: **applied** — `t-87` ships `withWriteConflictRetry` (bounded, idempotent body, only `P2034` retried); the concurrency model and its known scope limit are recorded in the Hub journal against §21._

### HB9 · A feature can ship its write surface ahead of its read surface — a write-only field is indistinguishable from a missing one (HCE Hub · f-phases)

- **Discovery.** `f-phases` was scoped as "verbs + reads + UI, no schema change", and its **reads were derived from one consumer's needs** — what Plan grouping required (name, status, feature count) — rather than from the model. Designing `f-work-kinds`/`f-phase-history` five weeks later surfaced the residue: **`Phase.description` is written** by `create_phase`/`update_phase`, **selected _and_ mapped** in `listProjectPhases` (`phases.ts:43,53`), and then **dropped at both boundaries** — no component references it, and the `list_phases` MCP projection omits it. Same for `startedAt`/`completedAt`, derived and maintained correctly in `phases-service` and rendered nowhere. The same shape had already bitten once elsewhere: `get_feature` returns spec, DoD, status, stage, deps and a task roll-up — but not the feature's phase, which the Plan row shows. The owner's report was _"there's no sense of intent or purpose beyond the name"_ — for a field that had existed, and been writable, the whole time.
- **Impact.** A write-only field is functionally identical to a missing one, but **costs more than a missing one**: it looks present in the schema and in the write API, so the gap never gets filed (nobody asks for a field that's right there), and it stays unpopulated because nothing surfaces it — circular, and silent. It also deferred the cost: caught at build it is one component + one projection; caught five weeks later it is spread across a band header, a management dialog, an MCP projection and a feature page, and it needed a design conversation to notice at all.
- **Feedback.** When a feature **adds or activates a model**, enumerate its fields and state, per field, both the **write path** and the **read + render site** — and treat a field with a write path and no render site as a **plan-level gap to surface** (the inverse of [HB8]: not "the design wants what the schema lacks" but "the schema has what nothing surfaces"). Concretely, at close-out run a `field → written-by → read-by → rendered-at` table for every model the feature touched; anything with an empty last column is either cut from the schema or earns a task. The tell for this class is a feature whose reads were scoped from a single consumer — ask "what would a _second_ consumer need?" before calling the read surface done. _Status: **applied** — `f-phase-history` (§33) carries the description render + MCP projection + lifecycle dates; the convention is recorded in [work-triage](./work-triage.md)._
