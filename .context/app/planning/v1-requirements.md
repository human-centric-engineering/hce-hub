...
- **Project** — id, name, hostPlatform, knowledgeCategoryId (for project-scoped RAG), repo URL(s), leadUserId, status (`planning` / `active` / `archived` — lifecycle state, distinct from operational focus state which lives on `FocusDirective`), createdAt
