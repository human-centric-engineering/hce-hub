...
*Feature 16 on [plan](./plan.md). A **corrective** feature: it closes a schema gap that originated in [f-data-model](./f-data-model.md) (§03) and was papered over in [f-plan-view](./f-plan-view.md) (§09). Binding design: `design/data.jsx` (`feature.id = 'f-data'`, `task.id = 't-6'`) + the design handoff Plan/Board card refs (`f-sidekick · t-6`). **Extends** [f-data-model](./f-data-model.md); **retrofits** [f-plan-view](./f-plan-view.md) (§09) + [f-board-view](./f-board-view.md) (§10). **Blocks the §10 close-out** (the board should ship with real refs).*
...
**Why this exists (the honest provenance).** [f-data-model](./f-data-model.md) modelled `Feature`/`Task` with cuid surrogate `id`s (the Sunrise convention) and no human key — the spec's §10 sketch used `id` generically, so no slug/number column was added. When the design's `f-slug · t-N` met that reality in [f-plan-view](./f-plan-view.md) t-2, it was **resolved unilaterally** as "the title carries identity" and filed as reconciliation bullet #5 — a **product/design decision made silently instead of surfaced.** The owner caught it. (Corollary bug: the `t-N` the Plan view *does* render is per-feature positional (`index+1`), which is *also* wrong — it's ephemeral, not the stable project-wide number the design intends.) **Retro lesson → [planning-retro](./planning-retro.md): a design element the schema lacks is a fork to surface for the owner's decision, not a reconciliation to settle in a feature doc.**
...
**None.** Pure leaf-app `app.prisma` migration + fork-owned retrofits. (Records: this feature also captures the **process retro lesson** — surface schema-gaps-vs-design as owner decisions, don't silently reconcile — in [planning-retro](./planning-retro.md) at close-out.)

## Decisions log   (append-only, newest first)

- **2026-07-15 — SHIPPED (owner Simon; PRs #51 schema+seed+write + #52 read+UI retrofit). Browser-validated (after a dev-server restart).** The correction is complete: `Feature.slug` (per-project unique, authored) + `Task.number` (project-wide, stable) + `Project.taskCounter` are in the schema; the **B13-stripped additive migration** applied (drift 15/15 — six spurious DROPs stripped); `create_task`/`add_backlog` assign numbers via the **atomic counter bump**; the seed backfills (dev DB verified: 15 slugs, 18 numbers, `taskCounter=18`); and `/plan` + `/board` + both UIs render the real `f-slug` / `t-{number}` refs (with null fallbacks). **`/code-review`:** `Task.number` is unique **by construction** (the counter), with no DB backstop — a documented invariant **carried to [§13](./plan.md#13-f-intake)** (`persist-features` must assign through the counter, not write `number` directly). **`/security-review`:** clean; also filed **[sunrise#437](https://github.com/human-centric-engineering/sunrise/issues/437)** — a `sanitizeUrl` whitespace/control-char bypass surfaced reviewing the board's PR-link (`bug`+`upstream-gap`; dormant for us, no `prUrl` write path). **Retro lesson (→ [planning-retro](./planning-retro.md)):** a design element the schema lacks is a fork to *surface* for the owner, not a reconciliation to settle silently. **Operational gotcha (not a code bug):** after the schema change, a long-running dev server holds a stale Prisma client → the new `select slug/number` throws a `PrismaClientValidationError` mapped to HTTP 400; a dev-server restart fixes it. **Pure leaf-app; sized to 2 PRs. Unblocked the [§10](./plan.md#10-f-board-view) close-out.**
- **2026-07-15 — Claimed + planned (owner Simon).** Corrective feature closing the feature-slug / task-number gap. Origin: [f-data-model](./f-data-model.md) modelled cuid ids only; [f-plan-view](./f-plan-view.md) §09 silently reconciled the design's `f-slug·t-N` to "title carries identity" (owner caught it). **Owner decisions:** slugs are **authored** (the plan-authoring AI writes them, short/scannable — not title-derived); **sequence = schema-fix PR then retrofit, before the §10 close-out.** Key reconciliations: `Feature.slug String?` per-project-unique + `Task.number Int?` project-wide via a `Project.taskCounter` atomic bump in the task-creating capabilities; fork-owned additive migration (B13 `--create-only` watch); retrofit `/plan` + `/board` reads + the plan/board UI to render real refs; seed backfills slugs/numbers/counter. **Pure leaf-app: no HB2, no platform edit, no upstream ask.** **Recommended 2 PRs.** **Blocks the §10 close-out.**
</content>
