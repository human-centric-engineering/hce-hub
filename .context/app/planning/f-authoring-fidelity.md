...
- **You can edit a task after planning.** A new `update_task` verb edits an existing task's `title`/`description`/`doneWhen`/`filesScope` (owner-tier via the feature funnel). Today there is **no way to amend a task over MCP** — so §20's `t-36`/`t-37` are stuck description-less. `update_task` is how they get backfilled (dogfooding the new verb, no direct DB touch).
...
**Sizing — recommended 3 PRs.** (t-a + t-b) the capture half (verb detail + `update_task`) read together as "the MCP write surface now round-trips"; (t-c) the task-sheet render; (t-d) the feature-side split + migration + cutover re-author (its own review surface + the migration). t-a and t-b can share one PR (both backend verb surface, no migration); t-c and t-d are separable render/schema work.
