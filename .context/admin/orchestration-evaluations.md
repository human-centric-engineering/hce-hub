...
1. **Create a dataset** (`/datasets/new`). Two paths via a Tabs control, with a sidebar "Anatomy of a case" panel that stays visible across both:
   ...
   - **Generate from description** — cold-start path (Phase 3.6). Pick a subject agent, type 20–1000 chars of domain prose ("Customer support for a fintech card issuer…"), optionally add 1–3 anchor user inputs, pick a target count. The preview endpoint (`/datasets/generate-from-description`) runs the eval-case-generator in `description` mode (no KB sampling, no failure seeds). Review → name → Save commits via `/datasets/generate-from-description/commit`, which creates the AiDataset row and writes accepted cases atomically.
