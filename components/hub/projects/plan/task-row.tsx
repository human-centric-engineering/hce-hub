 ...
 *
 * Columns: `t-N` · title · claimed-by · pr · effective status. Display-only in
 * §09 — clicking a row opens the task sheet in `f-task-sheet` §11 (not wired
 * here). A null claimer renders "—" (effective status already reconciles an
 * erased claimant back to the pool, so a null here means genuinely unclaimed).
 */
...
export function TaskRow({ task, ordinal }: { task: PlanTask; ordinal: number }) {
  ...
  return (
    <div
      className={`${TASK_ROW_GRID} border-t px-4 py-2 pl-7 text-[13px]`}
