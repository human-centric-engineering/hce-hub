 ...
 *
 * HCE Hub is Hub-first: `/` is the `(hub)` shell — the actual product — not the
 * account dashboard. Set here rather than at each door, which is what this seam
 * (sunrise#473, v0.8.0) exists for; before it, the Hub carried hand-edits to
 * `proxy.ts` and `login-form.tsx` (platform-divergence rows 12–13, now deleted).
 *
 * Every door follows, including the ones that used to stay on `/dashboard`
 * (signup, OAuth, invite acceptance, verify-email). That split existed to show
 * new users the dashboard's email-verification card — moot in production, where
 * `requireEmailVerification` means an unverified user never gets a session and
 * so never reaches this route at all: email signup goes to `/verify-email`, and
 * invite acceptance sets `emailVerified` before signing the user in.
 */
export const appAuthLandingRoute: string | null = '/';
 ...
 *
 * "Hub" — so the admin sidebar reads "Back to Hub" and the error pages' escape
 * hatch names the place it actually returns you to.
 */
export const appAuthLandingLabel: string | null = 'Hub';
