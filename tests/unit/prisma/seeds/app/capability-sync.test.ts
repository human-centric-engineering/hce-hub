...
import listPhasesUnit, { listPhasesFunctionDefinition } from '@/prisma/seeds/app/023-list-phases';
...
import updateIdeaUnit, { updateIdeaFunctionDefinition } from '@/prisma/seeds/app/025-update-idea';
...
describe('capability seeds re-sync functionDefinition on update', () => {
  ...
  it('023-list-phases: the new read capability seed re-syncs on the update branch', async () => {
    const { ctx, upsert } = runContext();
    await listPhasesUnit.run(ctx);
    const arg = upsert.mock.calls[0][0];
    expect(arg.where).toEqual({ slug: 'list_phases' });
    expect(arg.update.functionDefinition).toEqual(listPhasesFunctionDefinition);
    expect(arg.update.functionDefinition.parameters.properties).toHaveProperty('projectId');
  });

  it('010-create-feature: the update branch carries the current schema, including phaseId', async () => {
    const { ctx, upsert } = runContext();
    await createFeatureUnit.run(ctx);
    const arg = upsert.mock.calls[0][0];
    expect(arg.where).toEqual({ slug: 'create_feature' });
  ...
  it('025-update-idea: the new capability seed re-syncs on the update branch', async () => {
    const { ctx, upsert } = runContext();
    await updateIdeaUnit.run(ctx);
    const arg = upsert.mock.calls[0][0];
    expect(arg.where).toEqual({ slug: 'update_idea' });
    expect(arg.update.functionDefinition).toEqual(updateIdeaFunctionDefinition);
    expect(arg.update.functionDefinition.parameters.properties).toHaveProperty('status');
  });

  it('026-list-ideas: the new read capability seed re-syncs on the update branch', async () => {
    const { ctx, upsert } = runContext();
    await listIdeasUnit.run(ctx);
    const arg = upsert.mock.calls[0][0];
    expect(arg.where).toEqual({ slug: 'list_ideas' });
    expect(arg.update.functionDefinition).toEqual(listIdeasFunctionDefinition);
    expect(arg.update.functionDefinition.parameters.properties).toHaveProperty('projectId');
  });

  it('027-list-tasks: the new read capability seed re-syncs on the update branch', async () => {
    const { ctx, upsert } = runContext();
    await listTasksUnit.run(ctx);
    const arg = upsert.mock.calls[0][0];
    expect(arg.where).toEqual({ slug: 'list_tasks' });
    expect(arg.update.functionDefinition).toEqual(listTasksFunctionDefinition);
    expect(arg.update.functionDefinition.parameters.properties).toHaveProperty('kind');
  });

  it('028-get-task: the new read capability seed re-syncs on the update branch', async () => {
    const { ctx, upsert } = runContext();
    await getTaskUnit.run(ctx);
    const arg = upsert.mock.calls[0][0];
