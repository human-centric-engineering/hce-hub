...
describe('proxy (project root)', () => {
  ...
  describe('Auth routes — authenticated users', () => {
    it('redirects to the auth landing route when accessing /login with the HTTP cookie', async () => {
      ...
      expect(response.status).toBe(307);
      expect(new URL(response.headers.get('location')!).pathname).toBe(AUTH_LANDING_ROUTE);
      expect(response.headers.get('x-request-id')).toBe('test-request-id-123');
    });

    it('redirects to the auth landing route when accessing /signup with the HTTPS cookie', async () => {
    ...
    it('redirects to the auth landing route when accessing /reset-password with a session', async () => {
