...
| RAG (14)                              | Knowledge base + `search_knowledge_base` cap + `rag_retrieve` step (hybrid BM25 + vector). `AiAgent.knowledgeRetrievalMode` (`model` / `first_turn` / `every_turn` / `keywords`) picks when retrieval fires; `keywords` mode uses `knowledgeTriggerKeywords` for whole-word case-insensitive matching                                                                                                                                                                                                                  |
| Guardrails (18)                       | `guard` step type (LLM / regex / schema), plus per-agent input/output/citation guard modes                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Evaluation (19)                       | `evaluate` step type + `judge_call` step type (drives a judge agent inline; gates on `passed: boolean`) + per-agent named metrics (faithfulness / groundedness / relevance) + dataset-driven evaluation runs with model + pairwise judges (`workflow_as_judge`, `pairwise_judge_agent` graders) and a CI gate                                                                                                                                                                                                          |
